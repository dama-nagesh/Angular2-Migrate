...
angular.module('myApp.mf_capital_gain', ['ngRoute', 'oc.lazyLoad', 'ngLoadingSpinner', 'ngCsv', 'ui.bootstrap'])

    .config(['$routeProvider', function ($routeProvider) {
        $routeProvider.when('/mutual_fund/capital_gain', {
            templateUrl: 'modules/mutual_fund/capital_gain/capital_gain.html',
            controller: 'mf_capital_gainCtrl'
        });

    }])

    .controller('mf_capital_gainCtrl', ['$http', '$scope', '$window', '$location', 'dataService_mf_capital_gain', function ($http, $scope, $window, $location, dataService_mf_capital_gain) {
        ...
        function loadData(data, value) {
            $window.localStorage.setItem("mf_capital_gain", JSON.stringify(data));
        ...
        dataService_mf_capital_gain.getData('Actual').then(function (data) {
            $scope.apiData = data;
            ...
            $scope.filename = "MF_CapitalGain";
        });

        $scope.openModel = function (item) {
            $scope.mf_capital_gain_items = item.CapitalGainDetail;
        }

        $scope.toggleDiv = function (e) {
            if ($("#mfcg").hasClass("collapse")) {
                $("#mfcgdiv1").hide();
            }
            else {
                $("#mfcgdiv1").show();
        ...
        $scope.toggleTab = function (id1, id2, value) {
            $("#" + id1).show();
            $("#" + id2).hide();
            dataService_mf_capital_gain.getData(value).then(function (data) {
                $scope.apiData = data;
                loadData(data, value);
    ...
    .factory('dataService_mf_capital_gain', function ($q) {
        return {
