...
angular.module('myApp.eq_net_position', ['ngRoute', 'oc.lazyLoad', 'ngLoadingSpinner', 'ngCsv', 'ui.bootstrap'])

    .config(['$routeProvider', function($routeProvider) {
        $routeProvider.when('/equity/net_position', {
            templateUrl: 'modules/equity/net_position/net_position.html',
            controller: 'eq_net_positionCtrl'
        });
    } ])

    .controller('eq_net_positionCtrl', ['$http', '$scope', '$window', '$location', 'dataService_eq_net_position', function($http, $scope, $window, $location, dataService_eq_net_position) {
        ...
        dataService_eq_net_position.getData($scope.dataItemSelected).then(function(data) {
            $window.localStorage.setItem("eq_net_position", JSON.stringify(data));
            $scope.apiData = data;
            ...
            $scope.filename = "Equity_NetPosition";
            loadData(data);
        });

        //$("#netPositionTable").height($(window).height() - $(".page-header").height() - 230);
    ...
    .factory('dataService_eq_net_position', function($q) {
        return {
